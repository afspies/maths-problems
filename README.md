...
|---|---|---|---|---|
| [Four-dimensional Mahler volume conjecture](problems/mahler-volume-4d/PROBLEM.md) | 🟡 partial results | For every convex body K⊂R^4, prove \|K\| \|(K-s(K))°\| ≥ 3125/576, with equality only for simplices. | Sharp inequality for every 4D affine join; exact covariance formulas exclude eight smooth 24-cell curves; a q-regular stress arc yields and excludes an open 24-dimensional moduli family of smooth pair-terminal 24-cells. | — |
...
| [Square Peg conjecture](problems/square-peg/PROBLEM.md) | 🟡 partial results | Does every Jordan curve in the plane contain the four vertices of a nondegenerate square? | Finite-p-variation (p<2) and Dini-Young curves get every rectangle; a finite-2-variation positive-area Jordan curve proves embedded C0 primitive stability fails at the critical exponent | — |
| [Vizing’s domination conjecture](problems/vizing-domination/PROBLEM.md) | 🟡 partial results | Do all finite simple graphs satisfy γ(G □ H) ≥ γ(G)γ(H)? | K1/K3 terminal-capacity inequality; formal fibre-incidence/coordinate-hole classification; square-clique and rank-one tensor bounds (constant still 0.5643) | — |
