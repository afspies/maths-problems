...
|---|---|---|---|---|
| [Four-dimensional Kakeya conjecture](problems/kakeya-r4/PROBLEM.md) | 🟡 partial results | Must every Kakeya set in R^4 have Hausdorff and Minkowski dimension 4? | Full Hausdorff dimension for fixed transverse, rank-two-separated, and affine-rotating rank-one parabolic stacks; one-shot cover partitions eliminate transverse wall mass, with no new general or sticky bound | — |
| [Four-dimensional Mahler volume conjecture](problems/mahler-volume-4d/PROBLEM.md) | 🟡 partial results | For every convex body K⊂R^4, prove \|K\| \|(K-s(K))°\| ≥ 3125/576, with equality only for simplices. | Sharp inequality for every 4D affine join; every pair-terminal two-level 4-polytope is a simplex; terminal non-simplices have quadratic rank ≥3 and at least 8 vertices. | — |
...
| [Square Peg conjecture](problems/square-peg/PROBLEM.md) | 🟡 partial results | Does every Jordan curve in the plane contain the four vertices of a nondegenerate square? | A repaired unreviewed Tao-5.6 conormal argument passes internal audit, but an exact balanced family traps its full rank and action on artificial squares; positive GKS and integral boundary refinements also fail, leaving a finite odd-splice exclusion | — |
| [Vizing’s domination conjecture](problems/vizing-domination/PROBLEM.md) | 🟡 partial results | Do all finite simple graphs satisfy γ(G □ H) ≥ γ(G)γ(H)? | Componentwise equality calibration and a Cartesian support-avoidance tax exclude an exact connected (5,1,3) primitive and its formal mixture at zero defect (best constant still 0.5643) | — |
