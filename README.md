...
|---|---|---|---|---|
| [Four-dimensional Kakeya conjecture](problems/kakeya-r4/PROBLEM.md) | 🟡 partial results | Must every Kakeya set in R^4 have Hausdorff and Minkowski dimension 4? | Fixed-stack incidence theorem gives full Hausdorff dimension for a structured all-scale ruled-quadric subclass; exact extraction no-go and degenerate-quadric classification isolate the missing general bridge, with no new general or sticky bound | — |
| [Four-dimensional Mahler volume conjecture](problems/mahler-volume-4d/PROBLEM.md) | 🟡 partial results | For every convex body K⊂R^4, prove \|K\| \|(K-s(K))°\| ≥ 3125/576, with equality only for simplices. | Sharp inequality for every 4D affine join; terminal non-pyramids have at least six nonsimplicial facets and satisfy 2f03≥5(f0+f3)-4; universal unweighted quadratic and fixed-geometry flip ansatzes ruled out. | — |
...
| [Square Peg conjecture](problems/square-peg/PROBLEM.md) | 🟡 partial results | Does every Jordan curve in the plane contain the four vertices of a nondegenerate square? | Every conformal exhaustion has an L2 area lift; the null spiral sharply realizes fixed action at zero trace capacity and reduces the unrestricted problem to the derived extension of a vanishing-width microlocal eye | — |
| [Vizing’s domination conjecture](problems/vizing-domination/PROBLEM.md) | 🟡 partial results | Do all finite simple graphs satisfy γ(G □ H) ≥ γ(G)γ(H)? | Typed near-cover and fractional-energy inequalities; robust Cayley benchmark cleared at all scales; quantitative isolation-to-escape charging (best constant still 0.5643) | — |
